...
# pip install tensorflow scikit-image opencv-python numpy Pillow matplotlib
...
def load_and_preprocess_image(image_path: str, target_size: tuple) -> (np.ndarray, np.ndarray):
    ...
    Returns the normalized float array (Iprep) and the original resized 0-255 uint8 array.
    """
    ...
    # This `normalized_array` will serve as Iprep
    img_array_float = np.array(resized_img_pil, dtype=np.float32)
    normalized_array = (img_array_float / 255.0 * 2) - 1
    print(f"  Image resized to {target_size} and normalized to [-1, 1]. (Iprep created)")
...
def decompose_and_highlight_lesions(
    ...
    Returns dark_lesion_map (Idark) and bright_lesion_map (Ibri).
    """
...
def extract_features_with_xception(img_prep: np.ndarray, img_bri: np.ndarray, img_dark: np.ndarray) -> (tf.Tensor, tf.Tensor, tf.Tensor):
    """
    Extracts features using a pre-trained Xception model backbone.
    Inputs are expected to be (H, W, C) numpy arrays, where C=3 for img_prep
    and C=1 for img_bri and img_dark.
    Returns feature matrices (Mprep, Mbri, Mdark) of shape 14x14x1556.
    """
    print("\n  --- Starting Feature Extraction with Xception ---")
    # Load the Xception model pre-trained on ImageNet, without the top classification layer
    # The input_shape must match the TARGET_IMAGE_SIZE used for preprocessing.
    base_model = Xception(weights='imagenet', include_top=False,
                          input_shape=(TARGET_IMAGE_SIZE[0], TARGET_IMAGE_SIZE[1], 3))
    print("  Xception model backbone loaded successfully.")

    # Ensure inputs are in batch format (add batch dimension)
    # img_prep is already 3-channel (H, W, 3) and normalized [-1, 1]
    input_prep_for_xception = tf.expand_dims(img_prep, axis=0)
    # Xception's preprocess_input expects pixel values in [0, 255] or [0, 1] that it scales to [-1, 1]
    # Our normalized_img is already [-1,1]. Let's convert to [0,255] for preprocess_input's expected range
    # Or, if feeding directly to model that expects [-1,1] then remove preprocess_input
    # For robustness, let's convert to 0-255 and then use xception_preprocess_input.
    input_prep_for_xception = ((input_prep_for_xception + 1) / 2) * 255.0
    input_prep_for_xception = xception_preprocess_input(input_prep_for_xception)


    # Convert 1-channel grayscale maps (img_bri, img_dark which are [0,1]) to 3-channel
    # by stacking, and convert to [0,255] then preprocess for Xception.
    img_bri_3_channel = np.stack([img_bri, img_bri, img_bri], axis=-1)
    input_bri_for_xception = tf.expand_dims(img_bri_3_channel, axis=0)
    input_bri_for_xception = xception_preprocess_input(input_bri_for_xception * 255.0) # Scale [0,1] to [0,255]


    img_dark_3_channel = np.stack([img_dark, img_dark, img_dark], axis=-1)
    input_dark_for_xception = tf.expand_dims(img_dark_3_channel, axis=0)
    input_dark_for_xception = xception_preprocess_input(input_dark_for_xception * 255.0) # Scale [0,1] to [0,255]

    print("  Preparing inputs for Xception: Iprep, Ibri, Idark.")

    # Extract features
    Mprep = base_model(input_prep_for_xception)
    Mbri = base_model(input_bri_for_xception)
    Mdark = base_model(input_dark_for_xception)

    print(f"  Features extracted. Shapes: Mprep={Mprep.shape}, Mbri={Mbri.shape}, Mdark={Mdark.shape}")

    # Remove the batch dimension to return 14x14x1556 as specified
    return Mprep[0], Mbri[0], Mdark[0]
...
if __name__ == "__main__":
    ...
    images_to_process = [
        dummy_img_path_1, # Use dummy images for initial testing
        dummy_img_path_2,
        dummy_img_path_3,
        dummy_img_path_4,
        # "test_images/5.jpeg", # Uncomment and replace with your actual image paths
        # "test_images/6.jpeg",
        # "test_images/7.jpeg",
    ]
    ...
    for img_file_path in valid_images:
        ...
        # `normalized_img` here acts as Iprep (original image preprocessed)
        ...
        # dark_lesion_map acts as Idark
        # bright_lesion_map acts as Ibri
        ...
        # --- NEW: Feature Extraction with Xception ---
        print("\n--- Performing Feature Extraction ---")
        Mprep, Mbri, Mdark = extract_features_with_xception(
            img_prep=normalized_img, # Iprep
            img_bri=bright_lesion_map, # Ibri
            img_dark=dark_lesion_map # Idark
        )
        print(f"Extracted features shapes: Mprep={Mprep.shape}, Mbri={Mbri.shape}, Mdark={Mdark.shape}")
        # --- END NEW ---

        # Create heatmap overlays for visualization (existing code)
        ...
        # Prepare a single figure for output (existing code)
        ...
        axes[1].set_title("Dark Lesions (Red Lesions) Map")
        axes[1].axis('off')

        axes[2].imshow(cv2.cvtColor(bright_lesion_overlay_bgr, cv2.COLOR_BGR2RGB))
        axes[2].set_title("Bright Lesions (Exudates) Map")
        axes[2].axis('off')
