...
# Vessel Suppression Parameters
APPLY_VESSEL_SUPPRESSION = True  # Set to True to attempt to remove vessels before dark lesion detection
VESSEL_FILTER_LENGTH = 15  # Length of the matched filter for vessel detection. Adjust based on vessel thickness.
VESSEL_SUPPRESSION_STRENGTH = 0.8  # How strongly to suppress detected vessels (0.0 to 1.0). Higher = more suppression.
...
def suppress_vessels_matched_filter(image: np.ndarray, kernel_length: int) -> np.ndarray:
    """
    Suppresses vessels in a grayscale image using matched filtering.
    Based on responses from multiple orientations.
    """
    if kernel_length % 2 == 0:  # Ensure odd kernel length
        kernel_length += 1

    angles = np.arange(0, 180, 15)  # Check 12 angles from 0 to 165 degrees
    vessel_response = np.zeros_like(image, dtype=np.float32)

    for angle in angles:
        # Create a 2D Gaussian matched filter kernel
        kernel = cv2.getGaborKernel((kernel_length, kernel_length),
                                    sigma=kernel_length / 4.0,  # Sigma related to kernel size
                                    theta=angle * np.pi / 180.0,
                                    lambd=kernel_length / 2.0,  # Wavelength (related to vessel width)
                                    gamma=0.5,  # Spatial aspect ratio
                                    psi=0)  # Phase offset

        # Normalize kernel to sum to 0 (for enhancing features against background)
        kernel -= kernel.mean()

        # Apply filter
        filtered_img = cv2.filter2D(image.astype(np.float32), cv2.CV_32F, kernel)

        # Take the maximum response across all angles
        vessel_response = np.maximum(vessel_response, filtered_img)
...
def decompose_and_highlight_lesions(
    ...
    # --- Apply CLAHE Pre-processing (Optional) ---
    if apply_clahe:
    ...
    # --- Vessel Suppression (Optional) ---
    vessel_map = np.zeros_like(processed_green_channel, dtype=np.float32)
    processed_green_channel_for_lesions = processed_green_channel.astype(np.float32)
    if apply_vessel_suppression:
        print(
            f"  Applying vessel suppression (Filter Length: {vessel_filter_length}, Strength: {vessel_suppression_strength})...")
        vessel_map = suppress_vessels_matched_filter(processed_green_channel, vessel_filter_length)

        # Invert vessel map so vessels are bright, then suppress them in the original image
        # This reduces the intensity of pixels identified as vessels.
        # Scale vessel_map to 0-1 for blending
        vessel_map_scaled = vessel_map * vessel_suppression_strength

        # Reduce intensity of pixels where vessels are detected
        # Ensure we don't go below 0 after suppression
        processed_green_channel_for_lesions = np.clip(processed_green_channel_for_lesions * (1 - vessel_map_scaled), 0,
                                                      255)
        print("  Vessels suppressed in green channel.")
    else:
        print("  Vessel suppression skipped.")
    ...
    opened_bright = opening(processed_green_channel_for_lesions, se_bright)
    bright_lesion_map_raw = np.maximum(0, processed_green_channel_for_lesions - opened_bright)
    ...
    if bright_lesion_map.max() > 0:
        ...
        bright_lesion_map = np.zeros_like(bright_lesion_map)
    ...
    for current_radius in dark_se_radii:
        ...
        # Apply black top-hat to the vessel-suppressed green channel
        current_dark_map_raw = black_tophat(processed_green_channel_for_lesions, se_dark)

        temp_dark_map = current_dark_map_raw.astype(np.float32)
        if temp_dark_map.max() > 0:
            temp_dark_map /= temp_dark_map.max()
        all_scale_dark_maps.append(temp_dark_map)

    dark_lesion_map = np.zeros_like(processed_green_channel_for_lesions, dtype=np.float32)
    ...
    return dark_lesion_map, bright_lesion_map, vessel_map
...
if __name__ == "__main__":
    ...
    print(
        f"  Vessel Suppression: {'Enabled' if APPLY_VESSEL_SUPPRESSION else 'Disabled'} (Filter Length: {VESSEL_FILTER_LENGTH}, Strength: {VESSEL_SUPPRESSION_STRENGTH})")
    print(
    ...
    for img_file_path in valid_images:
        ...
        dark_lesion_map, bright_lesion_map, vessel_map = decompose_and_highlight_lesions(
            normalized_img,
            APPLY_CLAHE_PREPROCESSING, CLAHE_CLIP_LIMIT, CLAHE_TILE_GRID_SIZE,
            APPLY_VESSEL_SUPPRESSION, VESSEL_FILTER_LENGTH, VESSEL_SUPPRESSION_STRENGTH,
        ...
        # Prepare a single figure for output with 6 subplots (added vessel map)
        fig, axes = plt.subplots(1, 6, figsize=(30, 6))  # Increased figure size for 6 plots
        ...
        # 4. Pure Dark Lesion Mask (Heatmap)
        axes[3].imshow(dark_lesion_map, cmap='viridis', vmin=0, vmax=1)
        axes[3].set_title("4. Pure Dark Lesion Mask (Heatmap)")
        axes[3].axis('off')

        # 5. Pure Bright Lesion Mask (Heatmap)
        axes[4].imshow(bright_lesion_map, cmap='viridis', vmin=0, vmax=1)
        axes[4].set_title("5. Pure Bright Lesion Mask (Heatmap)")
        axes[4].axis('off')

        # 6. Vessel Map (from suppression step)
        axes[5].imshow(vessel_map, cmap='gray', vmin=0, vmax=1)  # Display vessel map in grayscale
        axes[5].set_title("6. Detected Vessel Map")
        axes[5].axis('off')
