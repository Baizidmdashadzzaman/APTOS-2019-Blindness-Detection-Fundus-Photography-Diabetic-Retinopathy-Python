...
if __name__ == "__main__":
    ...
    for img_file_path in valid_images:
        ...
        # 4. Pure Dark Lesion Mask (Now with heatmap colormap)
        # Use the same colormap as the overlay for consistency with 5.jpg style
        axes[3].imshow(dark_lesion_map, cmap='viridis', vmin=0, vmax=1)
        axes[3].set_title("4. Pure Dark Lesion Mask (Heatmap)")
        axes[3].axis('off')

        # 5. Pure Bright Lesion Mask (Now with heatmap colormap)
        # Use the same colormap as the overlay for consistency with 5.jpg style
